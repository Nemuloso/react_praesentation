...
uniform samplerCube envMipmaps[5];
...
vec4 HDRColor(in vec4 color){
	return vec4( ReinhardToneMapping( RGBEToLinear(color).xyz ), 1.0);
}
...
vec4 hdrTextureCubeLod(in vec3 directionVector, in float lod){
	...
	float ratio = lod - floor(lod);
	for(int i=0; i<6; i++){
		if(i==lowerIdx){
			vec4 lowerColor = textureCube(envMipmaps[i], directionVector);
			vec4 upperColor = textureCube(envMipmaps[i+1], directionVector);
			lowerColor = HDRColor(lowerColor);
			upperColor = HDRColor(upperColor);
			return vec4( lowerColor.rgb * (1.0 - ratio) + upperColor.rgb * ratio, 1.0);
		}
	}
}
...
void main(){
	...
	vec3 CmP		= cameraPosition - frag_position;
